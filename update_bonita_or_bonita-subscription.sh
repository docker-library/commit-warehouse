...
# bundle name suffix depending on Bonita minor version
BUNDLE_SUFFIX="tomcat"
if [ verlte "$MINOR" "7.8" ]
then
  BUNDLE_SUFFIX="Tomcat-${TOMCAT_VERSION}"
fi

echo "updating $EDITION"
case $EDITION in
"community")
        BUNDLE="BonitaCommunity-${NEW_RELEASE}-${BUNDLE_SUFFIX}"
        DIR="bonita"
        ...
        ;;
"subscription")
        BUNDLE="BonitaSubscription-${NEW_RELEASE}-${BUNDLE_SUFFIX}"
