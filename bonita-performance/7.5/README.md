...
First genereate a request key into a container with a specific hostname (-h):
	...
	docker run --rm --name=bonita -h bonita -ti bonitasoft/bonita-performance:7.4.0 /bin/bash
...
Retrieve the licence from the [customer portal](https://customer.bonitasoft.com) and place it to a directory on your host :	
    ...
    cp ~/Téléchargements/BonitaBPMSubscription-7.4-Cloud_Techuser-bonita-20170124-20170331.lic ~/Documents/Docker/Volumes/bonita-performance

Then we can launch the Bonita container with the same hostname (-h) and this host directory mounted (-v) :

	docker run --name bonita -h bonita -v ~/Documents/Docker/Volumes/bonita-performance/:/opt/bonita_lic/ -d -p 8080:8080 bonita
...
### MySQL
	...
	docker run --name bonita_mysql --link mydbmysql:mysql -h bonita -v ~/Documents/Docker/Volumes/bonita-performance/:/opt/bonita_lic/ -d -p 8080:8080 bonita

### PostgreSQL

	docker run --name mydbpostgres -e POSTGRES_PASSWORD=mysecretpassword -d bonitasoft/postgres
	docker run --name bonita_postgres --link mydbpostgres:postgres -h bonita -v ~/Documents/Docker/Volumes/bonita-performance/:/opt/bonita_lic/ -d -p 8080:8080 bonita

### Oracle

	docker run --name mydboracle -d alexeiled/docker-oracle-xe-11g
	docker run --name bonita_oracle --link mydboracle:oracle -e DB_ADMIN_PASS=oracle -e DB_NAME=xe -h bonita -v ~/Documents/Docker/Volumes/bonita-performance-7.2/:/opt/bonita_lic/ -d -p 8080:8080 bonitasoft/bonita-performance:7.2.1

## Modify default credentials

	docker run --name=bonita -e "TENANT_LOGIN=tech_user" -e "TENANT_PASSWORD=secret" -e "PLATFORM_LOGIN=pfadmin" -e "PLATFORM_PASSWORD=pfsecret" -h bonita -v ~/Documents/Docker/Volumes/bonita-performance/:/opt/bonita_lic/ -d -p 8080:8080 bonita
