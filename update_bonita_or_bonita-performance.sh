...
"community")
	...
	sed -i "s/^ENV BONITA_SHA256.*/ENV BONITA_SHA256 ${SHA256SUM}/" ${DIR}/7.0/Dockerfile
        cp /tmp/${BUNDLE}/bonita/engine-server/conf/platform/bonita-platform-community-custom.properties ${DIR}/7.0/templates/bonita-platform-community-custom.properties
        cp /tmp/${BUNDLE}/bonita/engine-server/conf/tenants/template/bonita-tenant-community-custom.properties ${DIR}/7.0/templates/bonita-tenant-community-custom.properties
        cp /tmp/${BUNDLE}/bonita/client/platform/conf/platform-tenant-config.properties ${DIR}/7.0/templates/platform-tenant-config.properties
...
"performance")
	...
	echo "We will update to : $NEW_RELEASE"
        ...
        cp /tmp/${BUNDLE}/bonita/engine-server/conf/tenants/template/bonita-tenant-community-custom.properties ${DIR}/7.0/templates/bonita-tenant-community-custom.properties
        cp /tmp/${BUNDLE}/bonita/client/platform/conf/platform-tenant-config.properties ${DIR}/7.0/templates/platform-tenant-config.properties
	cp /tmp/${BUNDLE}/bonita/engine-server/conf/platform/bonita-platform-sp-custom.properties ${DIR}/7.0/templates/bonita-platform-sp-custom.properties
        unzip -q -c /tmp/${BUNDLE}/webapps/bonita.war WEB-INF/web.xml > ${DIR}/7.0/files/WEB-INF/web.xml
        rm /tmp/${BUNDLE}.zip
        rm -rf /tmp/${BUNDLE}
        meld ${DIR}/7.0.bck ${DIR}/7.0
