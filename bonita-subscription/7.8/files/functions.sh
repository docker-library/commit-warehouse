...
function create_user_if_not_exists {
	...
	case "${db_vendor}" in
		...
		"mysql")
			...
			;;
		"oracle")
			echo "For Oracle schema is created with the 'create_database_if_not_exists' function."
			;;
	esac
}
...
function create_database_if_not_exists {
	...
	case "${db_vendor}" in
		...
		"mysql")
			...
			;;
		"oracle")
            ant -f ${BONITA_PATH}/bonita-deployer-databases-${BONITA_DBTOOL_VERSION}/deployer.xml init-oracle \
              -Ddb.server.name="$db_host" -Ddb.server.port="$db_port" -Ddb.instance.name="$db_name" -Ddb.user="$db_user" \
              -Ddb.password="$db_pass" -Ddb.admin.user="$db_admin_user" -Ddb.admin.password="$db_admin_pass" -Ddb.drop_existing="$db_drop_existing"
			;;
	esac
}
